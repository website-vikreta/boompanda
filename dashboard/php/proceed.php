<?php
    ...
    if($email && $userType && $id && ($userType == 'boompanda' || $userType == 'google' || $userType == 'facebook')){
        ...
        $check = "SELECT `id` FROM `offer_applications` WHERE `email` = '$email' AND `userType` = '$userType' AND `offerid` = '$id'";
        ...
        if(mysqli_num_rows($checkres) <= 0 ){
            ...
            if($row['name'] == "" || $row['mobile_number'] == "" || $row['current_address'] == "" || $row['college_name'] == "" || $row['dob'] == ""){
                header('location: ../offers-u.html?flag=profile');
            }
            ...
            if($row1['offer_type'] == 'free'){
                ...
                $sql = "INSERT INTO `offer_applications`(`name`, `email`, `userType`, `offerid`, `total_redeem`, `dateOfApplication`) 
                        VALUES ('".$row['name']."', '$email', '$userType', '$id', '".$row1['redeem_count']."', '$date')";
            ...
            header('location: ../offers-u.html?flag=fail');
        }
    }else{
        header('location: ../offers-u.html');
    }