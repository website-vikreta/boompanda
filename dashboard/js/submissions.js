    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (var i = 0; i < response[1].length; i++) {
                response[1][i].details = (response[1][i].details == "") ? "NA" : response[1][i].details;
    ...
    if (submission_type == 'all') {
        for (var i = 0; i < submissions.length; i++) {
            if (submissions[i].status == 'accepted') {
                ...
                var divEntry = "<div class='divEntry1 my-3' id='card-" + submissions[i].id + "' title='" + submissions[i].deleteReason + "'> <p class='p-0 m-0 font-weight-bold'>" + submissions[i].name + "</p> <p class='p-0 m-0 text-muted'>" + submissions[i].college + "</p><hr class='my-2'><div class='flex-between wrapper'><div div class='content'><p class='p-0 m-0 small'>" + submissions[i].email + "</p><p class='p-0 m-0 small'>" + submissions[i].mobile + "</p><p class='p-0 m-0 small'>" + submissions[i].city + ", " + submissions[i].state + "</p></div><a href='" + submissions[i].proofs + "' target='_BLANK' class='btn btn-sm' title='Check Out Proof'><i class='far fa-file-export'></i></a><a href='" + submissions[i].details + "' target='_BLANK' class='btn btn-sm btn-danger text-light' title='Check Out Details'><i class='far fa-eye'></i></a></div><hr class='mb-2'><div class='flex-between'><div class='status small text-primary'>" + submissions[i].status + "</div><div class='buttons'><button class='btn btn-sm rounded solid' id='reject-" + submissions[i].id + "' onclick='rejectSubmission(" + submissions[i].id + ")'><i class='far fa-times'></i></button><button class='btn btn-sm btn-solid ml-2 rounded' id='accept-" + submissions[i].id + "' onclick='acceptSubmission(" + submissions[i].id + ")'><i class='far fa-check'></i></button></div></div><hr class='my-2'><p class='text-danger small p-0 m-0 reason'>" + submissions[i].deleteReason + "</p></div>";
                $("#view-submissions-modal #task-container").append(divEntry);
            }
        }
        ...
        for (var i = 0; i < submissions.length; i++) {
            if (submissions[i].status == 'accepted') {
                var divEntry = "<div class='divEntry1 my-3' id='card-" + submissions[i].id + "' title='" + submissions[i].deleteReason + "'> <p class='p-0 m-0 font-weight-bold'>" + submissions[i].name + "</p> <p class='p-0 m-0 text-muted'>" + submissions[i].college + "</p><hr class='my-2'><div class='flex-between wrapper'><div div class='content'><p class='p-0 m-0 small'>" + submissions[i].email + "</p><p class='p-0 m-0 small'>" + submissions[i].mobile + "</p><p class='p-0 m-0 small'>" + submissions[i].city + ", " + submissions[i].state + "</p></div><a href='" + submissions[i].proofs + "' target='_BLANK' class='btn btn-sm' title='Check Out Proof'><i class='far fa-file-export'></i></a><a href='" + submissions[i].details + "' target='_BLANK' class='btn btn-sm btn-danger text-light' title='Check Out Details'><i class='far fa-eye'></i></a></div><hr class='mb-2'><div class='flex-between'><div class='status small text-primary'>" + submissions[i].status + "</div><div class='buttons'><button class='btn btn-sm rounded solid' id='reject-" + submissions[i].id + "' onclick='rejectSubmission(" + submissions[i].id + ")'><i class='far fa-times'></i></button></div></div><hr class='my-2'><p class='text-danger small p-0 m-0 reason'>" + submissions[i].deleteReason + "</p></div>";
                $("#view-submissions-modal #task-container").append(divEntry);
            }
        }
